...
REM   and its subdirectories, logging the name and content of each file that has
REM   a ".tf" extension.
REM - If the path is a file and has a ".tf" extension, it logs its name and 
REM   content directly.
...
SET "OUTPUT_FILE=TerraformFileContents.txt"
...
FOR /F "usebackq tokens=*" %%A IN ("%LIST_FILE%") DO (
    ...
    IF EXIST "%%A\" (
        ECHO "%%A" is a directory. Logging contents of .tf files within...
        ECHO "%%A" is a directory. Printing contents of .tf files within... >> "%OUTPUT_FILE%"
        ECHO. >> "%OUTPUT_FILE%"
        ...
        IF EXIST "%%A" (
            REM Check if the file has a .tf extension (case-insensitive).
            IF /I "%%~xA" == ".tf" (
                ECHO "%%A" is a .tf file. Logging its content...
                ...
                ECHO "%%A" is not a .tf file. Skipping.
            )
        ) ELSE (
